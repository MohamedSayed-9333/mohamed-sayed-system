...
with tab3:
        ...
        with col_f1:
            # استخدام قائمة منسدلة مع جلب البيانات من Postgres
        ...
        filter_sql = ""
        if sel_loc != "الكل": 
            filter_sql += f" WHERE u.location = '{sel_loc}'"
        ...
        # --- 4. سجل المبيعات مع التعديل والحذف ---
        st.write("---")
        st.subheader("🛒 سجل مبيعات الوحدات")
        ...
        if not df_sales.empty:
            ...
            with st.form("edit_sale"):
                ...
                new_price = st.number_input("تعديل السعر الإجمالي:", value=float(rec_s['السعر']))
                
                c1, c2 = st.columns(2)
                if c1.form_submit_button("💾 تحديث عملية البيع"):
                    conn = get_db_connection(); cur = conn.cursor()
                    cur.execute("""
                        UPDATE sales 
                        SET buyer_name=%s, sale_price=%s, remaining_amount = %s - paid_amount 
                        WHERE id=%s
                    """, (new_buyer, new_price, new_price, int(s_id)))
                    conn.commit(); conn.close(); st.success("تم التحديث"); st.rerun()
                
                if c2.form_submit_button("🗑 حذف عملية البيع"):
                    conn = get_db_connection(); cur = conn.cursor()
                    cur.execute("DELETE FROM sales_installments WHERE sale_id=%s", (int(s_id),))
                    cur.execute("DELETE FROM sales WHERE id=%s", (int(s_id),))
    ...
    with tab7:
        ...
        conn = get_db_connection()
        # ملخص المبيعات (استخدام ::date للمقارنة في Postgres)
...
# --- أرشيف المقاولات (PostgreSQL Version - Optimized) ---
